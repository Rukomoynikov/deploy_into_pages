...
const createPageDeployment = async ({ artifact_id, github_token, pages_build_version, oidc_token_url, oidc_bearer_token}: CreatePageDeploymentParams) => {
    ...
    if (!pageResponse.ok) throw new Error(`Response status: ${pageResponse.status}`);
...
const getDeploymentStatus = async (pages_deployment_id: string, github_token: string) => {
    const apiURL = process.env.GITHUB_API_URL ?? "https://api.github.com";
    var [owner, repo] = (process.env.GITHUB_REPOSITORY ?? "").split("/")

    const response = await fetch(`${apiURL}/repos/${owner}/${repo}/pages/deployments/${pages_deployment_id}`, {
        headers: {
            Accept: "application/vnd.github+json",
            Authorization: `Bearer ${github_token}`,
            'User-Agent': 'actions/oidc-client'
